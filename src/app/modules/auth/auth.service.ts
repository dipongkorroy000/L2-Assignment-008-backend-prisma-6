...
import generateToken from "../../helper/genarateToken";
...
const login = async (payload: Login) => {
  ...
  const accessToken = await generateToken({ email: user.email, role: user.role, secret: config.jwt_access_secret_key, expireIn: "1d" });
  const refreshToken = await generateToken({ email: user.email, role: user.role, secret: config.jwt_refresh_secret_key, expireIn: "30d" });
